...
- [x] 1. Repository hygiene (Phase 0)
...
- [x] 3. Bucket router fixes (Theme 4 → Requirement 1)
  ...
  - [x] 3.4 Add `CROSS_KEYWORD_SINGLE_WORDS` set constant in `bucket_router.py`
  ...
  - [x] 3.5 Implement `_extract_bucket_from_freetext(text)` helper
  ...
  - [x] 3.6 Update `extract_bucket_from_message` flow to integrate freetext layer
...
- [x] 5. GCS sync observability (Theme 2 → Requirement 2 + Requirement 8 GCS portion)
  ...
  - [x] 5.4 Refactor `_upload_db_to_gcs()` with same exception ladder
  ...
  - [x] 5.5 Wire upload calls in `create_or_update_user` and `close_database` (preserve existing call sites)
...
- [x] 6. Phase 2 LLM correctness (Theme 3 → Requirement 3)
  ...
  - [x] 6.4 Define `DEFAULT_STUFFING_MARKERS` constant and implement `_detect_default_stuffing(report)`
  ...
  - [x] 6.5 Update `enrich_with_media` to use new prompt + `max_tokens=6000` + `client_timeout=45s` + `asyncio.wait_for(50s)`
  ...
  - [x] 6.6 Wire fall-back-to-Phase-1 paths in `enrich_with_media` for truncation, timeout, default-stuffing
...
- [x] 8. /health endpoint extension (Theme 2.3 → Requirement 5 + 2)

  - [x] 8.1 Extend `HealthResponse` in `models.py` with `last_gcs_sync` and `build_marker`
  ...
  - [x] 8.2 Update `/health` handler in `main.py` to populate the new fields and apply the `degraded` rule
...
- [x] 10. Integration glue + main.py wiring (Phase 3)

  - [x] 10.1 Wire bucket router output through `_handle_bug_report` so the LLM receives the original brief
  ...
  - [x] 10.2 Verify Phase 2 fall-back paths each produce exactly one OpenProject ticket
  ...
  - [x] 10.3 Confirm env-validator warnings + `BUILD_MARKER` are emitted before any other startup line
    - Reorder `lifespan()` so the sequence is: `settings`, `validate_env_vars`, `BUILD_MARKER` log, `init_database`, clients init
    - Depends on 7.4
    - _Requirements: 5.1, 5.11_

- [x] 11. Synthetic webhook scenarios (Phase 4 → Theme 6 Tier 2)

  - [x] 11.1 Build common test harness in `scripts/synthetic_webhook.py`
  ...
  - [x] 11.2 Implement scenario S1 — empty brief + photo
  ...
  - [x] 11.3 Implement scenario S2 — `[LMS Webview] flickering` brief
  ...
  - [x] 11.4 Implement scenario S3 — `Login fails [step 3]` brief (negative test for over-matching)
  ...
  - [x] 11.5 Implement scenario S4 — 20-frame video bug
  ...
  - [x] 11.6 Implement scenario S5 — photo-only with default-stuffed Phase 2 response
  ...
  - [x] 11.7 Implement scenario S6 — registration survives mocked GCS round-trip
  ...
  - [x] 11.8 Implement scenario S7 — RC2 env-var corruption reproduction
  ...
  - [x] 11.9 Implement scenario S8 — truncated Phase 2 response triggers fall-back
  ...
  - [x] 11.10 Implement scenario S9 — Phase 2 timeout triggers fall-back (depends on 6.6)
  ...
  - [x] 11.11 Wire `--scenario all` to run S1..S9 sequentially with non-zero exit on any failure
...
- [x] 12. Secret hygiene (Phase 5 → Requirement 6)
  ...
  - [x]* 12.5 Test the pre-commit hook with a fake key (must reject) and a placeholder (must allow)
...
- [x] 13. Local end-to-end verification (Phase 6)
  ...
  - [x] 13.2 Run synthetic webhook scenarios green: `python scripts/synthetic_webhook.py --scenario all`
